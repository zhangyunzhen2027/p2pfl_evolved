...
class Stage:
    ...
    @staticmethod
    def execute(**kwargs) -> type["Stage"] | None:
        """
        Execute the stage.

        Concrete stages must override this method. It receives the same
        keyword arguments that `StageWorkflow.run()` was called with
        (typically including `state`, `learner`, `communication_protocol`,
        `aggregator`, etc.).
        """
...
def check_early_stop(state: NodeState, raise_exception: bool = True) -> bool:
    ...
    Raises:
        EarlyStopException: If `raise_exception` is True and early stopping
            is requested.
    """
    if state.round is None:
        logger.info(state.addr, "Stopping workflow.")
        if raise_exception:
