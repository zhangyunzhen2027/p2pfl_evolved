        ...
        **kwargs,
        ...
        if self.state.round is not None:
            ...
            try:
                ...
                base_model = self.learner.get_model()
                ...
                if staleness < 0:
                    staleness = 0
                raw_weight = kwargs.get("num_samples", kwargs.get("weight", 1))
                try:
                    raw_weight_float = float(raw_weight)
                except Exception:
                ...
                if effective_weight <= 0:
                    effective_weight = 1
                received_model = base_model.build_copy(
                    params=params,
                    num_samples=effective_weight,
                    contributors=[source],
                    additional_info=extra_info,
                )
                self.state.neighbor_models[source] = received_model
