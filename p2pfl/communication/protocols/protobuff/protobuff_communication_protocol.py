...
class ProtobuffCommunicationProtocol(CommunicationProtocol):
    ...
    .. todo:: Decouple the heartbeat command.
    """
    ...
    def send(
        ...
        """
        try:
            self._neighbors.get(nei).send(
                msg,
                raise_error=raise_error,
                disconnect_on_error=remove_on_error,
            )
    ...
    @running
    def get_neighbor_degree(self, addr: str) -> int:
        """
        Get the observed interaction degree of a neighbor.

        This is a thin wrapper around Neighbors.get_degree so that stages
        don't have to know about the Neighbors implementation.
        """
        # Neighbors already implements degree counting via note_interaction()
        if hasattr(self._neighbors, "get_degree"):
            try:
                return int(self._neighbors.get_degree(addr))
            except Exception:
                return 0
        return 0
